...
secure_vars_file="${WORKSPACE}/configuration-secure/ansible/vars/developer-sandbox.yml"
extra_var_arg="-e@${extra_vars_file}"

if [[ $edx_internal == "true" ]]; then
    # if this is a an edx server include
    # the secret var file
    extra_var_arg="-e@${secure_vars_file} -e@${extra_vars_file}"
fi
...
cat $secure_vars_file $extra_vars_file | grep -v -E "_version|migrate_db" | tee ${extra_vars_file}_clean
